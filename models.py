...
class Product(Base):
    ...
    cost_cn_usd: Mapped[float] = mapped_column(Float, default=0.0)        # purchase cost in China
    default_cnke_ship_usd: Mapped[float] = mapped_column(Float, default=0.0) # per-piece CN->KE ship
    profit_ads_budget_usd: Mapped[float] = mapped_column(Float, default=0.0) # fallback budget
...
class Warehouse(Base):
    ...
    country: Mapped[str] = mapped_column(String, nullable=False)     # human-readable name
...
class StockMovement(Base):
    ...
    product_sku: Mapped[str] = mapped_column(String, ForeignKey("products.product_sku"), nullable=False)
    from_wh: Mapped[int] = mapped_column(Integer, ForeignKey("warehouses.id"), nullable=True)
    to_wh: Mapped[int] = mapped_column(Integer, ForeignKey("warehouses.id"), nullable=True)
...
class Shipment(Base):
    ...
    from_country: Mapped[str] = mapped_column(String, nullable=False) # human-readable (e.g., Kenya)
    ...
    created_date: Mapped[str] = mapped_column(String, nullable=True)  # ISO
...
class DailyDelivered(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    date: Mapped[str] = mapped_column(String, nullable=False)          # ISO date
    country_code: Mapped[str] = mapped_column(String, nullable=False)  # KE, TZ, ...
...
class PeriodRemit(Base):
    ...
    cost_unit_usd: Mapped[float] = mapped_column(Float, default=0.0)        # CN cost + CN->KE + KE->Dest
