...
function calculateShippingCostPerPiece(db, productId, targetCountry) {
  ...
  // Find all shipments for this product that have arrived
  ...
  function traceCost(currentCountry, accumulatedCost = 0, visited = new Set()) {
    ...
    // Find shipments arriving at current country
    const incomingShipments = relevantShipments.filter(s => s.toCountry === currentCountry);
    ...
    for (const shipment of incomingShipments) {
      ...
      if (shipment.fromCountry === 'china') {
        ...
        traceCost(shipment.fromCountry, newAccumulatedCost, new Set(visited));
      }
...
function calculateProductCostPerPiece(db, productId) {
  ...
  // Find all China shipments for this product that have arrived
