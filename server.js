// server.js – EAS Tracker backend (Complete Rebuild)
...
function calculateProductCosts(db, productId, country = null) {
  ...
  const productShipments = shipments.filter(s => s.productId === productId && s.arrivedAt);
  ...
  productShipments.forEach(shipment => {
    if (!country || shipment.toCountry === country) {
      ...
      const cost = +shipment.chinaCost || 0;
      totalCost += cost;
      totalPieces += pieces;
    }
  });
...
function calculateDeliveryRate(db, productId, country, startDate, endDate) {
  ...
  const totalDelivered = periodDeliveries.reduce((sum, d) => sum + (+d.delivered || 0), 0);
...
function calculateProfitMetrics(db, productId, country, startDate, endDate) {
  ...
  const maxCostPerLead = deliveryRate > 0 ? (profit / totalPieces) * (deliveryRate / 100) : 0;
...
function calculateDateRange(rangeType) {
  ...
  switch(rangeType) {
    case '8days':
      startDate = new Date(now.setDate(now.getDate() - 8)).toISOString().slice(0,10);
      break;
    case '15days':
      startDate = new Date(now.setDate(now.getDate() - 15)).toISOString().slice(0,10);
      break;
    case '1month':
      startDate = new Date(now.setMonth(now.getMonth() - 1)).toISOString().slice(0,10);
      break;
    case '2months':
      startDate = new Date(now.setMonth(now.getMonth() - 2)).toISOString().slice(0,10);
      break;
    case '6months':
      startDate = new Date(now.setMonth(now.getMonth() - 6)).toISOString().slice(0,10);
      break;
    case '1year':
      startDate = new Date(now.setFullYear(now.getFullYear() - 1)).toISOString().slice(0,10);
      break;
    case '2years':
      startDate = new Date(now.setFullYear(now.getFullYear() - 2)).toISOString().slice(0,10);
      break;
    case 'lifetime':
...
app.post('/api/product-orders', requireAuth, (req,res)=>{
  ...
  const { productId, country, date, orders } = req.body||{};
  if (!productId || !country || !date) return res.status(400).json({ error:'Missing fields' });
...
app.get('/api/product-costs-analysis', requireAuth, (req,res)=>{
  ...
  const totalChinaShipping = shipments
    .filter(s => s.fromCountry === 'china')
    .reduce((sum, s) => sum + (+s.chinaCost||0), 0);
  
  const totalInterShipping = shipments
    .filter(s => s.fromCountry !== 'china')
    .reduce((sum, s) => sum + (+s.shipCost||0), 0);
  
  const totalExpenses = totalChinaShipping + totalInterShipping + totalInfluencerCost + metrics.totalProductCost;
...
app.post('/api/shipments', requireAuth, (req,res)=>{
  ...
  if (s.fromCountry === 'china' && !s.chinaCost) return res.status(400).json({ error:'China cost required for shipments from China' });
...
app.get('/api/analytics/remittance', requireAuth, (req,res)=>{
  const db = loadDB();
  const { range, start, end, country, productId } = req.query||{};
  
  const dateRange = range && range !== 'custom' ? calculateDateRange(range) : { startDate: start, endDate: end };
  
  let remittances = db.remittances||[];
  if (dateRange.startDate) remittances = remittances.filter(r=>r.start>=dateRange.startDate);
  if (dateRange.endDate) remittances = remittances.filter(r=>r.end<=dateRange.endDate);
  ...
  remittances.forEach(r => {
    ...
    if (!analytics[key]) {
      const metrics = calculateProfitMetrics(db, r.productId, r.country, dateRange.startDate, dateRange.endDate);
      analytics[key] = {
...
app.get('/api/analytics/profit-by-country', requireAuth, (req,res)=>{
  const db = loadDB();
  const { range, start, end, country } = req.query||{};
  ...
  countries.forEach(c => {
    const metrics = calculateProfitMetrics(db, null, c, dateRange.startDate, dateRange.endDate);
