...
app.post('/api/auth', (req, res) => {
  ...
  if (password === 'logout') {
    res.clearCookie('auth', {
      httpOnly: true,
      sameSite: 'None',   // important for iOS/Safari
...
// cascade delete product + related
...
app.post('/api/adspend', requireAuth, (req,res)=>{
  ...
  if (f) f.amount=+amount||0; else db.adspend.push({ id:uuidv4(), productId, country, platform, amount:+amount||0 });
  saveDB(db); res.json({ ok:true });
});
...
app.put('/api/shipments/:id', requireAuth, (req,res)=>{
  ...
  const s=(db.shipments||[]).find(x=>x.id===id); if (!s) return res.status(404).json({ error:'Not found' });
  const u=req.body||{};
...
app.get('/api/finance/entries', requireAuth, (req,res)=>{
  ...
  res.json({ entries:list, running: runBal(db), balance: perBal(list) });
});
...
app.post('/api/snapshots/restore', requireAuth, async (req,res)=>{
  ...
  // keep the snapshot (do NOT delete)
