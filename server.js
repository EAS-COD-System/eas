// server.js
// EAS Tracker backend (Render/Node 18+)
...
// ---------- countries ----------
app.get('/api/countries', requireAuth, (req,res)=>{ const db=loadDB(); res.json({countries:db.countries||[]}); });
...
app.delete('/api/countries/:name', requireAuth, (req,res)=>{
  ...
  if (n.toLowerCase()==='china') return res.status(400).json({error:'China cannot be deleted'});
  db.countries = (db.countries||[]).filter(c=>c!==n);
  saveDB(db); res.json({ ok:true, countries:db.countries });
});

// ---------- products ----------
app.get('/api/products', requireAuth, (req,res)=>{ const db=loadDB(); res.json({products:db.products||[]}); });
...
// ---------- daily ad spend ----------
app.get('/api/adspend', requireAuth, (req,res)=>{ const db=loadDB(); res.json({adSpends:db.adspend||[]}); });
...
// ---------- deliveries ----------
app.get('/api/deliveries', requireAuth, (req,res)=>{ const db=loadDB(); res.json({deliveries:db.deliveries||[]}); });
...
// ---------- shipments ----------
app.get('/api/shipments', requireAuth, (req,res)=>{ const db=loadDB(); res.json({shipments:db.shipments||[]}); });
...
app.post('/api/remittances', requireAuth, (req,res)=>{
  ...
  const r = {
    id:uuidv4(),
    start:req.body.start, end:req.body.end, country:req.body.country, productId:req.body.productId,
    orders:+req.body.orders||0, pieces:+req.body.pieces||0, revenue:+req.body.revenue||0,
    adSpend:+req.body.adSpend||0, extraPerPiece:+req.body.extraPerPiece||0
  };
...
app.post('/api/finance/categories', requireAuth, (req,res)=>{
  ...
  const {type,name} = req.body||{}; if(!type||!name) return res.status(400).json({error:'Missing type/name'});
...
app.get('/api/finance/entries', requireAuth, (req,res)=>{
  ...
  res.json({ entries:period, running:computeRunningBalance(db), balance:computePeriodBalance(period) });
});
...
// ---------- influencers ----------
app.get('/api/influencers', requireAuth, (req,res)=>{ const db=loadDB(); res.json({influencers:db.influencers||[]}); });
app.post('/api/influencers', requireAuth, (req,res)=>{
  const db=loadDB(); db.influencers=db.influencers||[];
  const {name,social,country}=req.body||{}; if(!name) return res.status(400).json({error:'Missing name'});
...
app.get('/api/influencers/spend', requireAuth, (req,res)=>{ const db=loadDB(); res.json({spends:db.influencerSpends||[]}); });
...
// ---------- snapshots ----------
app.get('/api/snapshots', requireAuth, (req,res)=>{ const db=loadDB(); res.json({snapshots:db.snapshots||[]}); });
